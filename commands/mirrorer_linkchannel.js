...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			interaction.reply({ content:'Discord channel ' + interaction.options.getChannel('dchannel').id +
                ' and forum board ' + interaction.options.getInteger('fboard') + ' have been linked.',
			ephemeral:true });
			db.end();
			return;
			...
			interaction.reply({ content:'Failed to connect to database.',
				ephemeral:true });
		}
